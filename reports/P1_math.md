...
工作分支 / code state：`codex/p0-p1-math-core` / P1 执行时基线 commit 为 `9cb4dce2bd5da81a65e795a7181e632c2e44edf1` 且新增实现尚未提交（dirty=true）。最终审查 commit 见 Git 交付。
...
| `.\\.venv\\Scripts\\python.exe -m pytest -q` | 0 | `21 passed in 5.89s`。 |
| `.\\.venv\\Scripts\\python.exe scripts/run_p1_checks.py --config configs/p1_smoke.json --output artifacts/p1_smoke_001` | 0 | 6 个固定 seed×gate smoke case，通过全部预定正确性阈值。 |

pytest 初次不可用是本机解释器缺少包，而不是用例跳过；在项目本地环境安装后运行。没有跳过的 P1 测试。

原始 smoke JSON 位于 `artifacts/p1_smoke_001/summary.json`（ignored）；已提交的小型摘要位于 `reports/p1_smoke_summary.json`。

## 结果

新增模块：

- `src/moe_event/toy.py`：完整 dense reference 与真正只执行所选 expert 的 sparse/event-only path；forced route 只固定 indices，在候选输入上重算 logits/softmax/gate。
...
- `src/moe_event/decomposition.py`：逐 token 的 `Delta L = Delta L_fixed + J_event`，新输入上的带符号交叉项与 jump²；不以旧输入或非负距离替代。
...
| 最大 dense/event-only actual 或 fixed output 差 | `8.881784197001252e-16` |
...
| event-only expert calls / case | 140–142 |
| event token 数 / case | 6–7 |
| `max|H - GN|` / case | `2.683e-4`–`1.980e-3` |

seed 2 的两个 gate mode 都出现负的 signed event（分别约 `-0.0078054`、`-0.0207986`），因此测试/结果不假定 event 非负。expert call 计数仅证明 toy path 未先 dense 后 mask；不是墙钟速度或部署效率主张。
...
| T09–T10 | fixed-scale legal/no-op/duplicate 与 shared-weight reachability；通过。 |
| T11–T12 | finite-difference grad、HVP、完整 H≠GN、事件后剩余 smooth Taylor error；通过。 |
| T13–T14 | dense/event-only parity 和实际 call 降低、tie/k/bias rejection；通过。 |
...
实际 smoke elapsed：`17.1930791s`；CPU only。峰值显存/host RAM：null（未测）。

P1 测试失败项：无（21 passed）。P1 smoke failure list：空。P0 legacy numeric mismatch 单列在 `reports/P0_environment.md`，没有被 P1 覆盖或解释为通过。
